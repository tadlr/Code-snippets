      ...
      if ($("fn:visible", context).length > 0) {
        ...
        $("fn:visible", context).each(function (index) {
          ...
          if (!filter_note || filter_note.length == 0) {
            ...
            let $desc = $(this).attr("text")
              ? $(this)
                  .attr("text")
                  .replaceAll("&lt;", "<")
                  .replaceAll("&gt;", ">")
              : this.innerHTML.replaceAll("&lt;", "<").replaceAll("&gt;", ">");
            ...
            footnotes[current_note_index]["count"] = parseInt(
              footnotes[current_note_index]["count"] + 1
            );
          }

          var footnote_id = "fn" + footnote_symbol;
          var footnote_reference_count = parseInt(
            footnotes[current_note_index]["count"]
          );
          var footnote_tag_id =
            footnote_reference_count > 1
              ? footnote_id + "-" + footnote_reference_count + "-rf"
              : footnote_id + "-rf";
          ...
          if (language == "en") {
            $(this).before(
              '<sup id="' +
                ...
                '"><a class="fn-lnk" href="#' +
                footnote_id +
                '"><span class="wb-inv">Footnote </span>' +
                footnote_symbol +
                "</a></sup>"
            );
          } else if (language == "fr") {
            $(this).before(
              '<sup id="' +
                ...
                '"><a class="fn-lnk" href="#' +
                footnote_id +
                '"><span class="wb-inv">Note de bas de page </span>' +
                footnote_symbol +
                "</a></sup>"
            );
          }
