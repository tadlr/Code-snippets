(function ($, Drupal) {
  Drupal.behaviors.generalBehaviors = {
    attach: function (context, settings) {
      ...
      if ($(".radio,.checkbox,.radio-inline,.checkbox-inline", context).length > 0) {
      ...
      if ($("details", context).not("[role=tabpanel]").length > 0) {
        $("details", context).not("[role=tabpanel]").each(function (index, element) {
            if ($(element).children(".details-wrapper").length == 0) {
              $(element).children().not("summary").wrapAll('<div class="details-wrapper"></div>');
            }
        });
      }
      ...
      // Print form
      $(once('printBtn', 'button,input[type=submit]', context)).each(function(){
